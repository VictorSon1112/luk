...
class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    // adding the target
    this.target = this.add.sprite(width / 2, height / 7, 'target');
    this.target.displayWidth = this.comp.api.screen_size.w / 1.8;
    this.target.scaleY = this.target.scaleX;
    this.target.depth = 1;

    this.thunder = this.add.sprite(width / 2.2, height / 3.5, 'thunder');
  ...
  changeSpeed() {
    ...
    // new rotation speed
    this.newRotationSpeed = Math.floor(
      (this.currentRotationSpeed * this.valuer.ship + variation) * sign
    );
        ...
        onComplete: function (tween) {
          ...
          // in case this is not a legal hit
            ...
            this.tweens.add({
              ...
              targets: [this.knife1],
              // y destination
              y: this.comp.api.screen_size.h * 2 + this.knife1.height,
